...
class AdaBoost:
    ...
    def update_weights(self, y, y_pred, sample_weights, alpha):
        ...
        negative_error = 1-err
        for i in range(len(sample_weights)):
            if y[i] == y_pred[i]:
                sample_weights[i] = sample_weights[i]/(2*negative_error)
            else:
    ...
    def predict(self, X):
        ...
        """
        stump_prediction = np.array([self.stumps[stump].predict(X) for stump in range(self.n_stumps)])
        return np.sign(stump_prediction[0])
